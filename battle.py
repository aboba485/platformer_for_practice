...
while running:
    ...
    for event in pg.event.get():
        if event.type == pg.QUIT:
            ...
            if event.key == pg.K_SPACE and char_y_char==310 and jumps_down == False:
                char_y_char -= jump
...
#-----------------------------------
    ...
    #колонна проитв персонажа
    if f_up_right == True or f_up_left_ == True:
        if char_x_char>=235:
            char_x_char = 235
    elif f_up_right == True or f_up_left_ == True:
        if char_x_char<=210:
            char_x_char = 210
